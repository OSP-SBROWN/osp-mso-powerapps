...
BEGIN
    ...
    SET @Hierarchy3_Name = NULLIF(LTRIM(RTRIM(@Hierarchy3_Name)), '');
    ...
    BEGIN
        SET @SQL_Message = N'Hierarchy3_Name is required.';
        RETURN;
    END;
    ...
    BEGIN
        SET @SQL_Message = N'Invalid ParentHierarchy_ID = '
            ...
            + N'; no such Hierarchy2 record for this version.';
        RETURN;
    END;
    ...
    BEGIN
        ...
        SET @SQL_Success = 0;
        SET @SQL_Message = N'Hierarchy3_Code ' + CAST(@Hierarchy3_Code AS NVARCHAR(20))
    ...
    BEGIN
        ...
        SET @NameWarning = N' WARNING: name "' + @Hierarchy3_Name
                         + N'" already exists (ID=' + CAST(@DupID3 AS NVARCHAR(20))
    ...
    BEGIN TRY
        ...
        DECLARE @Inserted TABLE (Hierarchy3_ID INT);

        INSERT INTO dbo.Hierarchy3
        (
            Hierarchy3_ID,               -- DEFAULT covers IDENTITY/sequence
            HierarchyVersion_ID,
            Hierarchy3_Code,
            Hierarchy3_Name,
            ParentHierarchy_ID,
        ...
        OUTPUT INSERTED.Hierarchy3_ID INTO @Inserted(Hierarchy3_ID)
        VALUES
        (
            DEFAULT,
            @HierarchyVersion_ID,
        ...
        SELECT @Hierarchy3_ID = Hierarchy3_ID FROM @Inserted;
    ...
    BEGIN CATCH
        ...
        BEGIN
            ...
            SELECT TOP (1)
                @ExistingID  = Hierarchy3_ID,
                @ExistingName = Hierarchy3_Name
            FROM dbo.Hierarchy3
            WHERE HierarchyVersion_ID = @HierarchyVersion_ID
              AND Hierarchy3_Code     = @Hierarchy3_Code;

            SET @SQL_Success = 0;
            SET @SQL_Message = N'Hierarchy3_Code ' + CAST(ISNULL(@Hierarchy3_Code,-1) AS NVARCHAR(20))
                             + N' already exists'
